...
################################################################################
print ("---PSSM-FREQUENCY MATRIX-BASED PREDICTOR---")
print ("Loading model...")
clf = joblib.load('FM.pkl')
...
for i in range(len(i_protname)):
    ...
    with open(protnamepath, 'r+') as PSSM:
        ...
        for everyline in pssm:
            ...
            tmp_sm2 = []
            for FM in tmp_sm:
                tmp_sm2.append(float(FM)/100)
            substitutionmatrix.append(tmp_sm2)
        combined_FM.append(substitutionmatrix)
    protcount = 0
    for protseq in combined_FM:
        protcount += 1
        countless = 0
        countmore = 0
        countrest = 0
        ...
        prediction = list(clf.predict(FM_Windows))
        prediction = [inv_ss_map[i] for i in prediction]
        prediction = ''.join(prediction)
        if (sys.argv[3] == 'y') or (sys.argv[3] == 'Y'):
            print (i_protname[protcount-1])
            print (prediction)
        pred.append(prediction)
################################################################################

print ("Writing prediction output into %s..." %(newpath+'FM_'+sys.argv[2]))
runtime = time.strftime("%Y-%m-%d %H:%M %Z")
with open((newpath+'FM_'+sys.argv[2]), 'w+') as op:
    op.write("HSCPred PSSM-Frequency matrix-based prediction "+runtime+'\n')
